...
def experiment(config: Dict[str, Union[int, float, List[str]]], train_data: pd.DataFrame, val_data: pd.DataFrame,
    ...
    tags = [f"FOLD_{fold_index}", "gemini", "token_num_balanced", "0421_fixed"] + extra_clearml_tags if extra_clearml_tags is not None else [
        f"FOLD_{fold_index}", "gemini", "token_num_balanced", "0421_fixed"]
    ...
    training_args = TrainingArguments(
        output_dir=f"regular_model_checkpoints {task_name_id}",
    ...
    shutil.rmtree(f"regular_model_checkpoints {task_name_id}", ignore_errors=False, onerror=None)

...
from data_preparation import random_misspell_text, random_extra_punctuations

tokenizer = BertTokenizer.from_pretrained("SZTAKI-HLT/hubert-base-cc")
pretrained_model_name = 'SZTAKI-HLT/hubert-base-cc'

CONFIG_PATH = "tdk_punctuations_config.json"
...
combine_feat_method_options = ["text_only", "concat", "individual_mlps_on_cat_and_numerical_feats_then_concat",
                               "attention_on_cat_and_numerical_feats", "gating_on_cat_and_num_feats_then_sum",
...
def experiment(config: Dict[str, Union[int, float, List[str]]], train_data: pd.DataFrame, val_data: pd.DataFrame,
    ...
    tags = [f"FOLD_{fold_index}", "gemini", "token_num_balanced", "punctuations", "0421_fixed"] + extra_clearml_tags if extra_clearml_tags is not None else [
        f"FOLD_{fold_index}", "gemini", "token_num_balanced", "punctuations", "0421_fixed"]
    ...
    best_models_dir = "best_models_TDK_3_classes"
    ...
    training_args = TrainingArguments(
        output_dir=f"punctuations_model_checkpoints {task_name_id}",
    ...
    shutil.rmtree(f"punctuations_model_checkpoints {task_name_id}", ignore_errors=False, onerror=None)
    ...
    for category_name, category in rates_by_categories.items():
        for drop_col in drop_col_options:
            ...
            for fold_index in range(NUMBER_OF_FOLDS):
                ...
                new_texts_col = []
                new_multiple_punct_num_col = []
                new_multiple_punct_rate_col = []
                for _, row in val_data.iterrows():
                    new_text, extra_multiple_punct_num = random_extra_punctuations(row["text"])
                    new_multiple_punct_num = row["multiplePunctNum"] + extra_multiple_punct_num
                    new_multiple_punct_rate = new_multiple_punct_num / row["punctNum"]
                    new_texts_col.append(new_text)
                    new_multiple_punct_num_col.append(new_multiple_punct_num)
                    new_multiple_punct_rate_col.append(new_multiple_punct_rate)

                val_data["text"] = new_texts_col
                val_data["multiplePunctNum"] = new_multiple_punct_num_col
                val_data["multiplePunctRate"] = new_multiple_punct_rate_col
